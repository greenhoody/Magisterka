...
\title{Sieć neuronowa dla gier typu fantasy football do konkursu BotBowl}
\engtitle{A neural network for fantasy football games for the BotBowl competition}
\album{299253}
...
Celem pracy było porównanie różnych architektur sieci neuronowych stosowanych w agentach uczących się gry w Blood Bowl z wykorzystaniem algorytmów typu actor–critic. W badaniach zastosowano dwa algorytmy uczenia ze wzmocnieniem: Advantage Actor-Critic (\acrshort{A2C}) oraz Proximal Policy Optimization (\acrshort{PPO}). Analizowano modele oparte na klasycznych warstwach konwolucyjnych, blokach Inception, architekturze ResNet, blokach Inception-Residual oraz mechanizmie hyper-connections. W badanych sieciach wykorzystano również techniki stabilizacji uczenia, takie jak Layer Normalization, inicjalizacja He, funkcja aktywacji Parametric ReLU (\acrshort{PReLU}) oraz maskowanie niedozwolonych akcji.
