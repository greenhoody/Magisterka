"""
Controlled residual inception policy without LayerNorm2d.
"""
...
class InceptionResidualBlock(nn.Module):
    def __init__(self, in_ch: int, kernels: Tuple[Tuple[int, int], ...] | list):
        ...
        if self.out_ch != in_ch:
            self.proj = nn.Conv2d(in_ch, self.out_ch, kernel_size=1, bias=True)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.inception(x)
        skip = x if self.proj is None else self.proj(x)
        return F.relu(y + skip, inplace=True)
...
class CustomPolicy(nn.Module):
    def __init__(
        ...
        if any(sum(out_ch for out_ch, _ in block_spec) != spatial_ch for block_spec in block_kernels):
            raise ValueError("Controlled variant requires the same output width in every block")

        self.spatial_stem = nn.Sequential(
            nn.Conv2d(c, spatial_ch, kernel_size=1, bias=True),
            nn.ReLU(),
        )
        self.spatial = nn.Sequential(
            *[InceptionResidualBlock(spatial_ch, block_spec) for block_spec in block_kernels]
        )
    ...
    def forward(self, spatial: torch.Tensor, non_spatial: torch.Tensor):
        ...
        spatial_feat = self.spatial(x).flatten(1)
