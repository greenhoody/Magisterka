...
class RolloutMemory:
    def __init__(
        ...
        self.old_log_probs = torch.zeros(t_steps, n_envs, 1, device=device)
    ...
    try:
        for update in range(1, total_updates + 1):
            for step in range(cfg.rollout_len):
                ...
                memory.old_log_probs[step].copy_(log_probs)
            ...
            flat_old_log_probs = memory.old_log_probs.reshape(batch_size, 1)
            flat_returns = returns.reshape(batch_size, 1)
            flat_advantages = advantages.reshape(batch_size, 1)

            mini_batch_size = max(1, batch_size // cfg.minibatches)
            value_losses = []
            policy_losses = []
            policy_entropies = []
            ...
            for _ in range(cfg.ppo_epochs):
                ...
                for start in range(0, batch_size, mini_batch_size):
                    ...
                    new_log_probs, new_values, policy_entropy = policy.evaluate_actions(
                        flat_spatial[idx],
                        flat_non_spatial[idx],
                        flat_actions[idx],
                        flat_action_masks[idx],
                    )

                    ratio = probability_ratio(
                        new_log_probs.squeeze(-1),
                        flat_old_log_probs[idx].squeeze(-1),
                    )

                    adv = flat_advantages[idx].squeeze(-1)
                    surr1 = ratio * adv
                    surr2 = (
                    ...
                    value_loss = F.mse_loss(new_values, flat_returns[idx])
                    loss = (
            ...
            value_loss_mean = torch.stack(value_losses).mean().item()
            policy_loss_mean = torch.stack(policy_losses).mean().item()
            policy_entropy_mean = torch.stack(policy_entropies).mean().item()
