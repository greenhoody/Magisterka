"""
Train an A2C agent that first learns versus the scripted `random` bot and then,
after a configurable number of policy updates, periodically swaps the opponent
to the most recent checkpoint of the learner itself.
"""
...
from a2c_residual_spatial_inception_agent import (
    A2CAgent,
    CNNPolicy,
...
# Environment
env_size = 7  # Options are 1,3,5,7,11
...
# Opponent schedule
initial_random_updates = 1000  # Keep random bot for this many updates
checkpoint_swap_interval = 500  # Swap opponent to latest checkpoint every N updates
...
class CheckpointOpponentScheduler:
    """Handles swapping scripted opponents to checkpoints on a fixed schedule."""

    def __init__(
        self,
        envs: "VecEnv",
        exp_identifier: str,
        keep_random_updates: int,
        swap_interval_updates: int,
    ):
        self.envs = envs
        self.exp_identifier = exp_identifier
        self.keep_random_updates = max(keep_random_updates, 0)
        self.swap_interval_updates = max(swap_interval_updates, 0)
        self.last_swap_update: Optional[int] = None
        self.swap_counter = 0

    def maybe_swap(self, update_idx: int, policy: nn.Module) -> Optional[str]:
        if self.swap_interval_updates <= 0:
            return None
        if update_idx < self.keep_random_updates:
            return None
        if (
            update_idx - self.keep_random_updates
        ) % self.swap_interval_updates != 0 or self.last_swap_update == update_idx:
            return None

        model_name = f"{self.exp_identifier}_opp_{self.swap_counter}.nn"
        model_path = os.path.join(model_dir, model_name)
        device = next(policy.parameters()).device
        cpu_policy = pycopy.deepcopy(policy).to(torch.device("cpu"))
        torch.save(cpu_policy, model_path)
        policy.to(device)
        opponent = make_agent_from_model(name=model_name, filename=model_path)
...
def main():
    ...
    if checkpoint_swap_interval > 0:
        checkpoint_scheduler = CheckpointOpponentScheduler(
            envs=envs,
            exp_identifier=exp_id,
            keep_random_updates=initial_random_updates,
            swap_interval_updates=checkpoint_swap_interval,
        )
    plot_suffix = "checkpoint" if checkpoint_scheduler else "random"
    ...
    while all_steps < num_steps:
        ...
        if checkpoint_scheduler is not None:
            swapped_path = checkpoint_scheduler.maybe_swap(all_updates, ac_agent)
            if swapped_path is not None:
                print(
                    f"[CheckpointScheduler] Swapped random opponent to {swapped_path} at update {all_updates}"
                )
        ...
        if all_updates % log_interval == 0 and len(episode_rewards) >= num_processes:
            ...
            fig.tight_layout()
            plot_name = f"{exp_id}_{plot_suffix}.png"
