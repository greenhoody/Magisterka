...
scripted `random` bot and, after a configurable number of updates, periodically
swaps the opponent to the latest checkpoint of the learner.
"""
...
# Opponent schedule
initial_random_updates = 1000
checkpoint_swap_interval = 500
...
class CheckpointOpponentScheduler:
    """Swap scripted opponents to latest learner checkpoints on a schedule."""
    ...
    def maybe_swap(self, update_idx: int, policy: nn.Module) -> Optional[str]:
        ...
        cpu_copy = pycopy.deepcopy(policy).to(torch.device("cpu"))
